...
def embedded_cnn(x):
    ...
    for n_gram, hidden_units in zip(KERNEL_SIZE, NUMBER_OF_FILTERS):
        ...
        conv_layer = Conv1D(
            filters=hidden_units, kernel_size=n_gram, padding="valid", activation="relu"
        )(emdded)
...
def get_model():
    ...
    word2index, embedding_matrix = load_glove_embeddings(
        fp=EMBEDDING_FILE_PATH, embedding_dim=EMBEDDING_DIM
    )
    ...
    fs = Dense(units=SIMILARITY_LAYER, activation="relu")(
        Concatenate()([f, s])
    )  # M_fs here

    # Similarity between second and third sentence
    # NOTE: I've used a dense layer instead of similarity matrix to get the similarity score
    st = Dense(units=SIMILARITY_LAYER, activation="relu")(
        Concatenate()([s, t])
    )  # M_st here
    ...
    model = Model(inputs=[x_1, x_2, x_3], outputs=is_coherent)
